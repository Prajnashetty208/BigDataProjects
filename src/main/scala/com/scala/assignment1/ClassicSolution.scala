package com.bigdata.assignment.assignment1

import java.io.FileWriter
import scala.collection.mutable
import scala.util.Random

object ClassicSolution {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Your solution ...
    // Below is the solution to the word count problem using Mutable Maps.
    val ourMutableMap: mutable.Map[String, Int] = mutable.Map[String, Int]()
    for (i <- ourStream) {
      if (ourMutableMap.contains(i)) ourMutableMap.update(i, ourMutableMap.get(i).get + 1)
      else ourMutableMap.update(i, 1)
    }
    println(ourMutableMap)

    //Dump to a csv file.
    val fw = new FileWriter("output1.csv")
